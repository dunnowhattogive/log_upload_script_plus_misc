...
def analyze_and_plot_results(writer, df, sheet_name, new_json=False):
    ...
    try:
        ...
        for _, row in df.iterrows():
            ...
            # Check for failures in Current Result, Test Result, and Voltage Result columns if new_json is True
            if new_json:
                if row.get("Current Result") == "FAILED":
                    row_failures += 1
                    reasons.append("Current Result failed")
                if row.get("Test Result") == "FAILED":
                    row_failures += 1
                    reasons.append("Test Result failed")
                if row.get("Voltage Result") == "FAILED":
                    row_failures += 1
            ...
            for cell in row:
                if "Failures" in ws.cell(row=1, column=cell.column).value:
                    if cell.value != "N/A" and str(cell.value).isdigit() and int(cell.value) > 0:
                        ...
                        cell.fill = pass_fill
                elif new_json and ws.cell(row=1, column=cell.column).value in ['Current Result', 'Test Result', 'Voltage Result']:
...
def main():
    ...
    data = process_json_file(directory, yearly, battery_test)
    aggregated_data = aggregated_data(data, yearly)
    
    # Save aggregated data to Excel
    excel_file = os.path.join(directory, "aggregated_data.xlsx")
    aggregated_data.to_excel(excel_file, index=False)
    
    # Upload to SMB
    upload_file_smb(excel_file, SERVER_NAME, SHARE_NAME, DESTINATION_PATH, USERNAME, PASSWORD)
